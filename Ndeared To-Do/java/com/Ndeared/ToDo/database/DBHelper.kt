...
class DBHelper private constructor(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun getAllTasks(): List<ModelTask> {
        ...
        val selectQuery = "SELECT  * FROM " + TASKS_TABLE
    ...
    fun getTasksForSearch(selection: String, selectionArgs: Array<String>, orderBy: String): List<ModelTask> {
        ...
        val c = db.query(TASKS_TABLE, null, selection, selectionArgs, null, null, orderBy)

        if (c.moveToFirst()) {
            do {
                val id = c.getInt(c.getColumnIndex(TASK_ID_COLUMN))
                val title = c.getString(c.getColumnIndex(TASK_TITLE_COLUMN))
                val date = c.getLong(c.getColumnIndex(TASK_DATE_COLUMN))
                val position = c.getInt(c.getColumnIndex(TASK_POSITION_COLUMN))
                val timeStamp = c.getLong(c.getColumnIndex(TASK_TIME_STAMP_COLUMN))
